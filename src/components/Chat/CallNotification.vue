<template>
    <div class="p-4 bg-black/80 rounded-xl">
        <div class="flex mb-4 items-center overflow-hidden">
            ...
            <picture class="border-2 block border-white basis-10 w-10 h-10 mr-2" v-else>
                <img :src="contact.img" clas="w-full h-full rounded-full object-cover" alt="" />
            </picture>
            <div class="overflow-hidden">
                <div class="font-medium text-white items-center whitespace-nowrap text-ellipsis overflow-hidden">
                    {{ contact.name }}
                </div>
        ...
        <AdvBanner v-if="banner" class="mb-3 bg-white/10 text-white border border-white/10 rounded" :banner="banner" />
...
import type { ContactItemType, BannerType } from '@/types/components.interface'
