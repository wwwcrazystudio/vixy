<template>
    <div>
        <div class="relative">
            <div class="md:bg-black/80 flex gap-3 md:gap-6 justify-center items-center md:p-4">
                <button class="rounded-full bg-white/40 w-11 h-11 grid place-content-center"
                ...
                <button class="rounded-full bg-red w-11 h-11 grid place-content-center">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
            ...
            <ul class="w-fit m-auto list-none bg-white rounded-md absolute left-0 right-0 bottom-14 md:bottom-20"
                v-if="showSettings" @click.stop>
                <li class="border-b border-black/10">
                    <button class="p-4 font-medium w-full text-left" @click="openModal('audio')">
                        Аудио
                    </button>
                </li>
                <li class="border-b border-black/10">
                    <button class="p-4 font-medium w-full text-left" @click="openModal('video')">
                        Видео
                    </button>
                </li>
                <li class="border-b border-black/10">
                    <button class="p-4 font-medium w-full text-left" @click="openModal('report')">
                        Сообщить о&nbsp;проблеме
                    </button>
                </li>
            </ul>
        ...
        <AudioSettings
            ...
            v-if="showAudioSettings" @close="showAudioSettings = false" />
        <VideoSettings
            class="absolute max-w-none rounded-b-none md:rounded-2xl md:fixed md:top-0 left-0 right-0 -bottom-[64px] md:bottom-0 z-50 !h-fit"
            v-if="showVideoSettings" @close="showVideoSettings = false" />
        <ReportModal
            class="absolute max-w-none rounded-b-none md:rounded-2xl md:fixed md:top-0 left-0 right-0 -bottom-[64px] md:bottom-0 z-50 !h-fit"
            v-if="showReportForm" @close="showReportForm = false" />

        <TransferModal
            class="absolute max-w-none rounded-b-none md:rounded-2xl md:fixed md:top-0 left-0 right-0 -bottom-[64px] md:bottom-0 z-50 !h-fit"
            v-if="showTransferForm" @close="showTransferForm = false" />
    </div>
</template>
