<template>
    <div class="flex bg-white h-screen overflow-hidden">
        <aside class="w-[72px] border-r flex flex-col border-gray p-4">
            <ul class="list-none">
                <GroupItem
        ...
        <div class="grow shrink flex">
            <div
                class="basis-[31%] overflow-hidden grow-0 shrink flex flex-col border-r border-gray"
            >
                ...
                <SearchInput
                    class="m-4 mb-2"
                    label="Пользователи, сообщения..."
                    v-model:value="searchQuery"
                />

                <ChatIcomingCall
                    class="m-4 mt-0"
                    :contact="coloredContacts[0]"
                    :banner="banner"
                />

                <div class="flex flex-col items-center m-auto" v-if="!contacts">
                    <svg
                        class="mb-6"
                        width="80"
                        height="80"
            ...
            <div class="grow basis-auto shrink flex bg-darkgray">
                <RouterView v-if="isSelectedContact" />
                <template v-else>
                    <div
                    ...
                    >
                        <div
                            class="w-38 h-38 mb-4 rounded-full grid place-content-center bg-white shadow-basic"
                        >
                            <svg
                                width="80"
                                height="80"
                    ...
                    <div
                        class="m-auto max-w-[290px] flex flex-col items-center"
                        v-else
                    >
                        <div
                            class="w-38 h-38 mb-6 rounded-full grid place-content-center bg-white shadow-basic"
                        >
                            <svg
                                width="80"
                                height="80"
            ...
            <ChatPopup class="absolute top-12 right-6">
                <div
                    class="bg-salad w-6 h-6 rounded-full absolute left-4 top-4 grid place-content-center"
                >
                    <svg
                        width="16"
                        height="12"
            ...
            <ChatPopup class="absolute top-36 right-6">
                <div
                    class="bg-red w-6 h-6 rounded-full absolute left-4 top-4 grid place-content-center"
                >
                    <svg
                        width="16"
                        height="16"
...
import GroupItem from '@/components/Chat/GroupItem.vue'
import ChatBtn from '@/components/Chat/IconBtn.vue'
...
const hasWidget = ref<boolean>(true)
    ...
    if (contacts) {
        if (query) {
            contacts = contacts.filter((el) => {
                ...
                // Find by message
                if (el.history.some((el) => el.content?.includes(query))) {
                    return true
                }
            ...
            contacts = contacts.map((el) => {
                el.history.forEach((historyItem) => {
                    const { content } = historyItem
                    if (content && content.includes(query)) {
                        const fromStart = content.indexOf(query)

                        const newContent = content.replace(query, () => {
                            let start = ''
                            let end = ''

                            start = content.substring(fromStart - 10, 10)
                            end = content.substring(
...
const isSelectedContact = computed(() => {
