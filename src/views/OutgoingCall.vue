<template>
    <div class="flex bg-dark h-screen overflow-hidden relative">
        <div class="basis-9/12 grow shrink relative overflow-hidden grid h-screen">
            <transition name="slide">
                <CallStatus class="absolute left-0 right-0 top-4 m-auto z-20" :status="status" v-if="showStatus" />
            </transition>
            ...
            <button
                ...
                :disabled="!hasCam">
                <svg :opacity="!hasCam ? '0.4' : 1" width="24" height="24" viewBox="0 0 24 24" fill="none"
            ...
            <CallReactions class="absolute left-6 top-40 z-20" :class="!hasCam && '!top-4'" />

            <CallVideo v-if="hasCam" class="w-full h-full absolute transition-all duration-1000 left-0 top-0 z-10"
                :class="videoPositionClass" :poster="video2" :status="status" :show="true" />

            <CallNoCam v-else v-show="!shiftVideo"
                class="absolute z-5 left-0 top-0 right-0 bottom-0 m-auto w-full h-full bg-dark" />

            <CallVideo class="w-full h-full object-cover object-center transition-all duration-1000"
        ...
        <CallSidebar @onSidebarExpanded="expandedSidebar = $event" :messages="messages" :banner="banner" class="z-40" />

        <CallFinished class="absolute left-0 top-0 right-0 bottom-0 w-full h-screen z-40"
            nickname="Unknown Purple Oyster" v-if="status === 'finished'" />
    </div>
</template>
