<template>
    <div class="grow shrink flex">
        <div class="grow basis-[31%] lg:max-w-[31%] xl:grow-0 p-4 shrink flex flex-col border-r border-gray"
            ...
            <ChatAdmin class="mb-4" :person="admin" v-if="admin" />
            ...
            <ul class="list-none overflow-auto scrollbar -mr-2.5 pr-2.5 @touchmove.stop">
                <li v-for="setting in filteredMenu" :key="setting.url">
                    <RouterLink class="flex px-0 md:px-2.5 py-2 rounded-lg" :to="`/chat/settings/${setting.url}`"
...
import type { ChatAdminType, SettingMenuItemType } from '@/types/components.interface.js';

// Placeholders
import { placeholderAdmin } from "@/placeholderData/chatAdmin";
import { placeholderSettings } from "@/placeholderData/chatSettings";
import { useRoute } from "vue-router";

const { isTablet } = useMobileDetection()
const route = useRoute()

const admin = ref<ChatAdminType>()
const searchQuery = ref<string>('')
const menu = ref<SettingMenuItemType[]>()

admin.value = placeholderAdmin
menu.value = placeholderSettings

const filteredMenu = computed(() => {
    if (searchQuery.value) {
        return menu.value?.filter(el => el.title.toLowerCase().includes(searchQuery.value.toLowerCase()))
    }
